  ...
  miscOptions: [
    {
      ...
      defaultValue: true,
      description:
        "Change photos in video. Used to prevent overriding wanted photos. (default: true)",
    },
    {
      name: "disableTTS",
      ...
      defaultValue: false,
      description:
        "Disable TTS in video. Used to prevent overriding wanted TTS. (default: false)",
    },
    {
...
export default function AdvancedOptions({
  ...
  setAdvancedOptions: React.Dispatch<React.SetStateAction<VideoOptions>>;
}) {
  // Modals
  ...
  async function fetchModels(aiType: string = selectedAIType.type) {
    ...
    try {
      let res = await fetch(
        `${BACKEND_ENDPOINT}/types/ai/models?type=${aiType}`,
      );
  ...
  async function fetchBgVideos() {
    ...
    try {
      ...
      if (!res.ok) {
        setIsBgVideosError(
          "Failed to fetch background videos: " +
      ...
      setIsBgVideosError(
        "Failed to fetch background videos due to internal error: " +
          (e.message ?? e.toString()),
      );
    }
  }
  ...
  async function fetchBgAudio() {
    ...
    try {
      ...
      if (!res.ok) {
        setIsBgAudioError(
          "Failed to fetch background audio: " +
            (data.error ?? data.toString()),
        );
      ...
      setIsBgAudioError(
        "Failed to fetch background audio due to internal error: " +
          (e.message ?? e.toString()),
      );
    }
  }
  ...
  const [selectedAIType, setSelectedAIType] = useState(
    config.aiOptions.types[0],
  );
  const [openAIEndpoint, setOpenAIEndpoint] = useState<undefined | string>(
    undefined,
  );
  ...
  const [selectedSubtitleModel, setSelectedSubtitleModel] = useState(
    config.subtitleOptions[0],
  );
  const [selectedOrientation, setSelectedOrientation] = useState(
    config.videoOptions.orientations[0],
  );
  const [miscOptions, setMiscOptions] = useState(
    config.miscOptions.map((option) => option.defaultValue),
  );
  ...
  // TTS Options
  const [ttsVoiceModel, setTTSVoiceModel] = useState<undefined | string>(
    undefined,
  );
  const [ttsModelMale, setTTSModelMale] = useState<undefined | string>(
    undefined,
  );
  const [ttsModelFemale, setTTSModelFemale] = useState<undefined | string>(
    undefined,
  );
    ...
    <div>
      ...
      <div className="space-y-4 mb-8">
        <div className="flex justify-between my-4">
          ...
          <Dropdown>
            <DropdownTrigger>
              <Button>{selectedAIType.name}</Button>
            </DropdownTrigger>

            <DropdownMenu
              onAction={(key) => {
                setSelectedAIType(
                  config.aiOptions.types.find((type) => type.type === key)!,
                );
                fetchModels(key.toString());
              }}
            >
              {config.aiOptions.types.map((type) => (
                <DropdownItem key={type.type} description={type.description}>
                  {type.name}
                </DropdownItem>
              ))}
            </DropdownMenu>
          ...
          <div className="flex items-center gap-2">
            <ButtonGroup>
              {isAiModelError ? (
                <>
                  <Button
                    color="danger"
                    variant="shadow"
                    onPress={modelModal.onOpen}
                  >
                    Failed to fetch models
                  </Button>
                  <Modal
                    isOpen={modelModal.isOpen}
                    onOpenChange={modelModal.onOpenChange}
                  >
                    <ModalContent>
                      {(onClose) => (
                        <>
                          <ModalHeader className="flex flex-col gap-1">
                            Erorr fetching models
                          </ModalHeader>
                          <ModalBody>
                            <p>{isAiModelError}</p>
                          </ModalBody>
                ...
                <Dropdown>
                  <DropdownTrigger>
                    <Button isLoading={selectedAIModel == ""}>
                      {selectedAIModel == ""
                        ? "Loading models"
                        : selectedAIModel}
                    </Button>
                  ...
                  <DropdownMenu
                    onAction={(key) => setSelectedAIModel(key.toString())}
                  >
                    {aiModels.map((model) => (
              ...
              <Tooltip content="Fetch AI models">
                <Button isIconOnly onClick={() => fetchModels()}>
                  Fetch models
                </Button>
              </Tooltip>
            </ButtonGroup>
      ...
      <div className="space-y-4 mb-8">
        <div className="flex justify-between my-4">
          ...
          <Dropdown>
            <DropdownTrigger>
              <Button endContent={<div>hello</div>}>
            ...
            <DropdownMenu
              onAction={(key) =>
                setSelectedTTSProvider(
                  config.ttsOptions.find((provider) => provider.type === key)!,
                )
              }
            >
              {config.ttsOptions.map((provider) => (
                <DropdownItem
                  key={provider.type}
          ...
          <>
            <div className="flex justify-between my-4">
              <div>
                ...
                <p className={subtitle({ size: "sm" })}>
                  Select the voice model (leave empty for default)
                </p>
              </div>
              <Input
                isClearable
            ...
            <div className="flex justify-between my-4">
              <div>
                ...
                <p className={subtitle({ size: "sm" })}>
                  Select the male voice name (leave empty for default)
                </p>
              </div>
              <Input
                isClearable
                className="w-56"
                placeholder="Enter male voice"
                startContent={<></>}
                onChange={(e) => setTTSModelMale(e.target.value)}
              />
            </div>
            <div className="flex justify-between my-4">
              <div>
                ...
                <p className={subtitle({ size: "sm" })}>
                  Select the female voice name (leave empty for default)
                </p>
              </div>
              <Input
                isClearable
                className="w-56"
                placeholder="Enter female voice"
                startContent={<></>}
                onChange={(e) => setTTSModelFemale(e.target.value)}
              />
            </div>
          </>
        )}
      ...
      <div className="space-y-4 mb-8">
        <div className="flex justify-between my-4">
          ...
          <Dropdown>
            <DropdownTrigger>
              <Button>{selectedImageType.name}</Button>
            </DropdownTrigger>

            <DropdownMenu
              onAction={(key) =>
                setSelectedImageType(
                  config.imageTypes.find((provider) => provider.type === key)!,
                )
              }
            >
              {config.imageTypes.map((provider) => (
                <DropdownItem
                  key={provider.type}
                  description={` ${provider.style === "ai" ? "(AI generated)" : "(Image search)"} - ${provider.description}`}
                >
                  {provider.name}
                </DropdownItem>
              ))}
          ...
          <>
            <div className="flex justify-between my-4">
              <div>
                ...
                <p className={subtitle({ size: "sm" })}>
                  Select the AI image model (leave empty for default)
                </p>
              </div>
              <Input
                isClearable
                className="w-56"
                placeholder="Enter AI image model"
            ...
            <div className="flex justify-between my-4">
              <div>
                ...
                <p className={subtitle({ size: "sm" })}>
                  Enter the AI image prompt (put styles, etc.) (leave empty for
                  default)
                </p>
              </div>
              <Input
      ...
      <div className="space-y-4 mb-8">
        <div className="flex justify-between my-4">
          ...
          <Dropdown>
            <DropdownTrigger>
              <Button>{selectedSubtitleModel.name}</Button>
            </DropdownTrigger>

            <DropdownMenu
              onAction={(key) =>
                setSelectedSubtitleModel(
                  config.subtitleOptions.find((model) => model.type === key)!,
                )
              }
            >
              {config.subtitleOptions.map((model) => (
                <DropdownItem key={model.type} description={model.description}>
                  {model.name}
                </DropdownItem>
              ))}
            </DropdownMenu>
        ...
        <div className="flex justify-between my-4">
          <div>
            ...
            <p className={subtitle({ size: "sm" })}>
              Maximum length for token (leave empty for default)
            </p>
          </div>
          <Input
            isClearable
            ...
            onChange={(e) => setSubLen(parseInt(e.target.value) ?? 0)}
          />
        </div>
        <div className="flex justify-between my-4">
          <div>
            ...
            <p className={subtitle({ size: "sm" })}>
              Font name for subtitles (leave empty for default)
            </p>
          </div>
          <Input
            isClearable
        ...
        <div className="flex justify-between my-4">
          <div>
            ...
            <p className={subtitle({ size: "sm" })}>
              Font size for subtitles (leave empty for default)
            </p>
          </div>
          <Input
            isClearable
        ...
        <div className="flex justify-between my-4">
          <div>
            ...
            <p className={subtitle({ size: "sm" })}>
              Font color for subtitles (hex with #) (leave empty for default)
            </p>
          </div>
          ...
          {showFontColorPicker ? (
            ...
            <Button onClick={() => setShowFontColorPicker(true)}>
              Choose Font Color
            </Button>
          )}
        </div>
        <div className="flex justify-between my-4">
          <div>
            ...
            <p className={subtitle({ size: "sm" })}>
              Stroke color for subtitles (hex with #) (leave empty for default)
            </p>
          </div>
          ...
          {showStrokeColorPicker ? (
            ...
            <Button onClick={() => setShowStrokeColorPicker(true)}>
              Choose Stroke Color
            </Button>
          )}
        </div>
        <div className="flex justify-between my-4">
          <div>
            ...
            <p className={subtitle({ size: "sm" })}>
              Stroke width for subtitles (leave empty for default)
            </p>
          </div>
          <Input
            isClearable
      ...
      <div className="space-y-4 mb-8">
        <div className="flex justify-between my-4">
          ...
          <Checkbox
            isSelected={useBgVideo}
            onValueChange={(e) => setUseBgVideo(e)}
          >
        ...
        <div className="flex justify-between my-4">
          ...
          <Checkbox
            isSelected={useBgMusic}
            onValueChange={(e) => setUseBgMusic(e)}
          >
            ...
            <ButtonGroup>
              <Dropdown>
                {isBgVideosError ? (
                  <>
                    <Button
                      color="danger"
                      variant="shadow"
                      onPress={bgVideoModal.onOpen}
                    >
                      Failed to fetch videos
                    </Button>
                    <Modal
                      isOpen={bgVideoModal.isOpen}
                      onOpenChange={bgVideoModal.onOpenChange}
                    >
                      <ModalContent>
                        {(onClose) => (
                          <>
                            <ModalHeader className="flex flex-col gap-1">
                              Erorr fetching videos
                            </ModalHeader>
                            <ModalBody>
                              <p>{isBgVideosError}</p>
                            </ModalBody>
                  ...
                  <DropdownTrigger>
                    <Button isLoading={selectedBgVideo == ""}>
                      {selectedBgVideo == ""
                        ? "Loading backgrounds"
                        : selectedBgVideo}
                    </Button>
                ...
                <DropdownMenu
                  ...
                  onAction={(key) => setSelectedBgVideo(key.toString())}
                >
                  {bgVideos.map((bg) => (
                    <DropdownItem key={bg}>{bg}</DropdownItem>
                  ))}
                </DropdownMenu>
              </Dropdown>
              <Tooltip content="Fetch background videos">
                <Button isIconOnly onClick={() => fetchBgVideos()} />
              </Tooltip>
          ...
          <div className="flex justify-between my-4">
            ...
            <ButtonGroup>
              <Dropdown>
                {isBgAudioError ? (
                  <>
                    <Button
                      color="danger"
                      variant="shadow"
                      onPress={bgAudioModal.onOpen}
                    >
                      Failed to fetch audio
                    </Button>
                    <Modal
                      isOpen={bgAudioModal.isOpen}
                      onOpenChange={bgAudioModal.onOpenChange}
                    >
                      <ModalContent>
                        {(onClose) => (
                          <>
                            <ModalHeader className="flex flex-col gap-1">
                              Erorr fetching audio
                            </ModalHeader>
                            <ModalBody>
                              <p>{isBgAudioError}</p>
                            </ModalBody>
                  ...
                  <DropdownTrigger>
                    <Button isLoading={selectedBgAudio == ""}>
                      {selectedBgAudio == ""
                        ? "Loading sounds"
                        : selectedBgAudio}
                    </Button>
                ...
                <DropdownMenu
                  className="max-h-[50vh] overflow-y-auto"
                  onAction={(key) => setSelectedBgAudio(key.toString())}
                >
                  {bgAudio.map((sound) => (
                    <DropdownItem key={sound}>{sound}</DropdownItem>
                  ))}
                </DropdownMenu>
              </Dropdown>
              <Tooltip content="Fetch background sounds">
                <Button isIconOnly onClick={() => fetchBgAudio()}>
                  Fetch audio
                </Button>
              </Tooltip>
        ...
        <div className="flex justify-between my-4">
          <div>
            ...
            {selectedOrientation != "vertical" ? (
              <Chip className="mt-2" color="danger" variant="shadow">
                Horizontal orientation may produce issues due to WIP
              </Chip>
            ) : null}
          </div>
          <Dropdown>
            <DropdownTrigger>
              <Button>{selectedOrientation}</Button>
            </DropdownTrigger>

            <DropdownMenu
              onAction={(key) => setSelectedOrientation(key.toString())}
            >
              {config.videoOptions.orientations.map((orientation) => (
                <DropdownItem key={orientation}>{orientation}</DropdownItem>
              ))}
            </DropdownMenu>
          </Dropdown>
        </div>
      ...
      <div className="space-y-4 mb-8">
        {config.miscOptions.map((option) => (
          <div key={option.name} className="flex justify-between my-4">
            ...
            <Checkbox
              key={option.name}
              isSelected={miscOptions[config.miscOptions.indexOf(option)]}
              onValueChange={(e) => {
          ...
          onPress={() => {
            setAdvancedOptions({
              ...
              internalOptions: {
                ...
                changePhotos:
                  miscOptions[
                    config.miscOptions.findIndex(
                      (option) => option.name === "changePhotos",
                    )
                  ],
                disableTTS:
                  miscOptions[
                    config.miscOptions.findIndex(
                      (option) => option.name === "disableTTS",
                    )
                  ],
                disableSubtitles:
                  miscOptions[
                    config.miscOptions.findIndex(
                      (option) => option.name === "disableSubtitles",
                    )
                  ],
                useMock:
                  miscOptions[
                    config.miscOptions.findIndex(
                      (option) => option.name === "useMock",
                    )
                  ],
              },
              ttsOptions: {
